(function () {
  ...
  var config = window.ETHAN_BOOSTER_CONFIG || {};
  function portalAccessToken() { return window.EthanPortalAccess ? window.EthanPortalAccess.getToken() : ''; }
  var portalReady = window.EthanPortalAccess && window.EthanPortalAccess.ready
    ? window.EthanPortalAccess.ready
    : Promise.resolve('');
  var assignmentId = config.assignmentId || 'eth-skill-booster';
  ...
  function installHeader() {
    ...
    header.innerHTML =
      '<a class="portal-brand" href="' + escapeHtml(config.returnUrl || '../') + '">Ethan SAT</a>' +
  ...
  function renderProgressReview(data) {
    ...
    state.studentName = data.studentName || 'Ethan';
  ...
  function jsonp(action, parameters, success, failure) {
    ...
    var callbackName = '__ethanBooster' + Date.now() + Math.random().toString(16).slice(2);
